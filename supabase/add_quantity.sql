-- Adds a dedicated `quantity` field to bobbleheads — the number given away, shown
-- in the detail info grid so people can gauge how rare a bobblehead is. Free text
-- so "25,000", "~15,000", or "Unknown" all work. Idempotent — safe to run more
-- than once. Paste into the Supabase SQL editor.
--
-- 1. Quantity columns on the three tables that carry bobblehead text fields.
...
-- 2. Approving a new_bobblehead submission now copies its quantity into the
