    ...
    <main className="min-h-full bg-slate-50 px-4 py-8 text-zinc-900 sm:px-8">
      ...
      <div className="mx-auto max-w-5xl">
        {!isLoading && items.length > 0 ? (
          <div className="mt-6 flex items-center gap-3">
            <input
              ...
              className="w-full max-w-sm rounded-lg border border-black/10 bg-white px-3 py-2 text-sm text-zinc-900 outline-none focus:border-accent"
            />
        ...
        {isLoading ? (
          ...
          <p className="mt-8 text-sm text-zinc-600">No {noun} yet.</p>
        ) : filtered.length === 0 ? (
          <p className="mt-8 text-sm text-zinc-600">No {noun} match your search.</p>
        ) : (
          <ul
            className={
              variant === "photo"
          ...
          >
            {filtered.map((item) => (
              ...
              >
                <Link href={item.href} className="flex gap-3 p-3 transition hover:bg-black/[0.04]">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-black text-zinc-900">{item.title}</p>
                    <p className="mt-0.5 truncate text-xs font-semibold uppercase tracking-wide text-accent">
                      {teamName(item.teamSlug)}
                    </p>
                    {item.owner ? (
                      <p className="mt-1 truncate text-xs text-zinc-600">{item.owner.name}</p>
                    ) : null}
                  </div>
                </Link>
              </li>
