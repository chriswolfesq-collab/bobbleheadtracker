...
function StatCard({
  ...
  return (
    <div className="rounded-lg border border-black/10 bg-white p-5">
      <p className="text-xs font-black uppercase tracking-wide text-zinc-600">{label}</p>
    ...
    <main className="min-h-full bg-slate-50 px-4 py-8 text-zinc-900 sm:px-8">
      ...
      <div className="mx-auto max-w-5xl">
        {isLoadingStats ? (
          ...
          !error ? <p className="mt-8 text-sm text-zinc-600">No stats available.</p> : null
        ) : (
          <>
            <SectionHeading>Listings by team</SectionHeading>
            <div className="mt-4 rounded-lg border border-black/10 bg-white p-2">
              {listingsByTeam === null ? (
                <p className="p-3 text-sm text-zinc-600">Loading…</p>
              ) : listingsByTeam.length === 0 ? (
                <p className="p-3 text-sm text-zinc-600">No listings yet.</p>
              ) : (
                <ul>
                  <li className="flex items-center gap-3 px-3 py-2 text-xs font-black uppercase tracking-wide text-zinc-600">
                  ...
                  {listingsByTeam.map((team) => (
                    <li
                      key={team.slug}
                      className="flex items-center gap-3 border-t border-black/[0.06] px-3 py-2 text-sm"
                    >
                      <Link
                        href={`/teams/${team.slug}`}
                        className="min-w-0 flex-1 truncate font-black uppercase tracking-wide text-accent hover:text-accent-hover"
                      >
                        {teamName(team.slug)}
                      </Link>
                  ...
                  {(() => {
                    ...
                    return (
                      <li className="flex items-center gap-3 border-t-2 border-black/10 px-3 py-2 text-sm font-black uppercase tracking-wide">
                        <span className="min-w-0 flex-1 text-zinc-800">Total</span>
                        <span className="w-20 shrink-0 text-right tabular-nums text-zinc-800">
                          {fmt(total)}
                        </span>
                        <span className="w-28 shrink-0 text-right tabular-nums text-zinc-600">
            ...
            <div className="mt-4 rounded-lg border border-black/10 bg-white p-2">
              {stats.top_teams.length === 0 ? (
                <p className="p-3 text-sm text-zinc-600">No owned bobbleheads yet.</p>
              ) : (
                <ul>
                  {stats.top_teams.map((team) => (
                    ...
                    >
                      <Link
                        href={`/teams/${team.slug}`}
                        className="font-black uppercase tracking-wide text-accent hover:text-accent-hover"
                      >
                        {teamName(team.slug)}
                      </Link>
                      <span className="tabular-nums text-zinc-800">{fmt(team.count)}</span>
                    </li>
                  ))}
                </ul>
              )}
            ...
            <div className="mt-4 rounded-lg border border-black/10 bg-white p-2">
              {stats.most_reported.length === 0 ? (
                <p className="p-3 text-sm text-zinc-600">No pending reports.</p>
              ) : (
                <ul>
