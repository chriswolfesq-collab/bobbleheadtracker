    ...
    <main className="min-h-full bg-slate-50 px-4 py-8 text-zinc-900 sm:px-8">
      ...
      <div className="mx-auto mt-6 max-w-4xl space-y-4">
        {isLoadingRows ? (
          ...
          <p className="text-sm text-zinc-600">No dead images. Every listing photo loaded on the last sweep.</p>
        ) : filtered.length === 0 ? (
          <p className="text-sm text-zinc-600">No images match your search.</p>
        ) : (
          filtered.map((row) => {
            const href =
            ...
            return (
              <div
                key={row.id}
                className={`grid gap-4 rounded-lg border bg-white p-4 sm:grid-cols-[auto_auto_1fr_auto] ${
              ...
              >
                ...
                <div className="min-w-0 text-sm">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-1 block truncate text-xs text-zinc-600 underline hover:text-accent-hover"
                  >
                    {row.image_url}
                  </a>
